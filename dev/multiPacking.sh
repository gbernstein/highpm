#!/usr/bin/zsh
#SBATCH --job-name=Packing                                               # Job name
#SBATCH --cpus-per-task=4                                                # CPUs per task
#SBATCH --mem=32gb                                                       # Job memory request
#SBATCH --time=48:00:00                                                  # Time limit hrs:min:sec
#SBATCH --output=/home2/vwetzell/ProperMotion_RetII/packing_auxnew00/out_%a.log   # Output log (must exist before submit)
#SBATCH --error=/home2/vwetzell/ProperMotion_RetII/packing_auxnew00/err_%a.err    # Error log (must exist before submit)
#SBATCH -p low                                                           # Partition
#SBATCH -q low                                                           # Remove if using bhuv_compute
#SBATCH --array=0-N                                                    # Job array range (= len(retii_healpix.npy) - 1)
#SBATCH --exclude=node[01-12]

AUXDIR=/home2/vwetzell/ProperMotion_RetII/packing_auxnew00
source /home2/vwetzell/gitrepos/highpm/dev/slurm_common.sh

python /home2/vwetzell/gitrepos/highpm/scripts/detectionPacking.py \
    --index ${SLURM_ARRAY_TASK_ID} \
    --healpix-npy /home2/vwetzell/ProperMotion_RetII/retii_healpix.npy \
    --exposures-file /home2/vwetzell/gitrepos/pixmappy/pixmappy/data/delveExposures.hdf5 \
    --detection-path /home2/vwetzell/ProperMotion_RetII/PositionCorrectedExposureCatalog/ \
    --output-path /home2/vwetzell/ProperMotion_RetII/HealpixDetectionCatalog/ \
    --overwrite

finish
