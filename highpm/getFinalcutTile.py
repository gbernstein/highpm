...
def getTileOf(ra, dec):
    ...
    query = (
        "SELECT tilename FROM y6a1_coaddtile_geom WHERE {:f} BETWEEN uramin AND uramax "
        + " AND {:f} BETWEEN udecmin AND udecmax"
    ).format(ra, dec)
    conn = ea.connect()
...
def getTileBounds(tilename):
    ...
    # First get the tile that it's in
    query = (
        "SELECT racmin,racmax,deccmin,deccmax,ra_cent,dec_cent "
...
def getFinalcutCatalog2(ramin, ramax, decmin, decmax):
    ...
    query = (
        ...
        + "{:f}) c INNER JOIN y6a1_catalog i on i.filename=c.filename"
    ).format(ramin, ramax, decmin, decmax)
    conn = ea.connect()
    tab = conn.query_to_pandas(query)
    return Table.from_pandas(tab)
...
def projectCatalog(cat, ra0, dec0, exposureTable, color=0.6):
    ...
    try:
        cat.add_columns(
            [zz, zz, zz, zz, zz], names=["XI", "ETA", "MJD", "PAR_XI", "PAR_ETA"]
        )
    except:
    ...
    cat.remove_rows(cat["MJD"] < 0.0)
    return


if __name__ == "__main__":
    help = (
        "Obtain finalcut catalog for use in finding proper motions.\n"
