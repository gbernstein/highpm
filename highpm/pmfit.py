...
def singleFit(xy,cov_xy, t,par_xy, parallax_prior=None):
    ...
    xyMean = np.mean(xy, axis=1)
    xy -= xyMean[:,np.newaxis]
    # Build matrices
    npts = xy.shape[1]
    ...
    A = np.array( [[one, zero, t, par_xy[:,0], zero],
                   [zero, one, t, zero, par_xy[:,1]]])
    ...
    alpha = np.einsum("ikm,ij,jkn",A,invC,A)
    beta = np.einsum("ikm,ij,kj",A,invC,xy)
    # Add parallax prior
    if parallax_prior is not None:
        alpha[4,4] += parallax_prior**-2
    # Solve for 5 parameters
    p = np.solve(alpha, beta)

    # Calculate fit and chisq per point
    fit =  np.dot(A,p)
    resid = xy - fit
    chisq = np.einsum("ik,ij,jk",resid,invC,resid)
